<?php
function astoryofgrace_preprocess_html(&$vars) {
...
function astoryofgrace_js_alter(&$javascript) {
  // Update jquery version for non-administration pages  
  if (arg(0) != 'admin' && arg(0) != 'panels' && arg(0) != 'ctools'  && !(module_exists('jquery_update'))) {
    $jquery_file = drupal_get_path('theme', 'astoryofgrace') . '/js/jquery-1.9.1.min.js';
    $jquery_version = '1.9.1';
    $migrate_file = drupal_get_path('theme', 'astoryofgrace') . '/js/jquery-migrate-1.2.1.min.js';
    $migrate_version = '1.2.1';
	$form_file = drupal_get_path('theme', 'astoryofgrace') . '/js/jquery-form-3.31.0.min.js';
    $form_version = '3.31.0'; 
...
function astoryofgrace_preprocess_page(&$vars) { 
...
function astoryofgrace_preprocess_block(&$vars) {
...
function astoryofgrace_region_has_block($region) {
...
function astoryofgrace_get_span($block_count, $block_id, $count_sidebars) {
  ...
  if ($count_sidebars) {
    ...
    if (astoryofgrace_region_has_block('sidebar_first')) {
      $available_width = $available_width - 0;
    }

    if (astoryofgrace_region_has_block('sidebar_second')) {
      $available_width = $available_width - 0;
    }
  }
 ...
 */
function astoryofgrace_status_messages($variables) {
...
function astoryofgrace_form_alter(&$form, &$form_state, $form_id) {
  if ($form_id == 'search_block_form') {
    ...
    $form['actions']['submit']['#src'] = drupal_get_path('theme', 'astoryofgrace') . '/images/searchbutton.png';
  }
}

/* Returns HTML for primary and secondary local tasks. */
function astoryofgrace_menu_local_tasks(&$vars) {
  $output = '';
...
function astoryofgrace_menu_local_task($vars) {
...
function astoryofgrace_menu_tree(&$vars) {
...
function astoryofgrace_menu_link(array $vars) {

  $element = $vars['element'];

  if (astoryofgrace_is_in_nav_menu($element)) {
    $sub_menu = '';
...
function astoryofgrace_is_in_nav_menu($element) {
  ...
  if (!isset($nav_theming_functions)) {
    ...
    $nav_theming_functions = array_map('astoryofgrace_block_id_to_function_name', $ids);
...
function astoryofgrace_block_id_to_function_name ($id) {
...
function astoryofgrace_breadcrumb(&$variables) {
  $output = '';
